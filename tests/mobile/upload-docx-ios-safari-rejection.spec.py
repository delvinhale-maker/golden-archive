"""
iOS Safari rejection-path regression: the AI Prompt Pack publish flow must
show a clear, user-facing rejection message when the manuscript file is
not one of the accepted types (.pdf, .txt, .json, .docx).

This is the WebKit / iOS Safari counterpart to
upload-docx-android-rejection.spec.py. It exercises the same client-side
validator through a Safari UA + WebKit engine + iPhone viewport so we
catch any browser-specific rejection regressions.

Run:
    python3 tests/mobile/upload-docx-ios-safari-rejection.spec.py
...
async def advance_to_step_two(page) -> None:
    ...
    await title_input.fill("iOS Safari Rejection Prompt Pack")
    ...
    await desc.fill(
        ...
        "the publish flow so this iOS Safari rejection regression test can "
...
async def main() -> None:
    async with async_playwright() as pw:
        ...
        await page.screenshot(path=str(SCREENSHOTS / "docx_ios_rejection_step2.png"))
        ...
        try:
            ...
            await page.screenshot(path=str(SCREENSHOTS / "docx_ios_rejection_fail.png"))
            fail(
                f"Expected rejection message not shown on iOS Safari. Wanted exactly:\n"
                f"  {EXPECTED_MESSAGE}"
            )
        ok(f"iOS Safari rejection message rendered: {EXPECTED_MESSAGE!r}")
        ...
        if await success.count() > 0 and await success.first.is_visible():
            fail(
                "UploadSuccess rendered with the .exe filename on iOS Safari — "
                "rejected file was incorrectly accepted into the prompt pack flow."
            )
        ok("UploadSuccess did NOT render for the rejected .exe on iOS Safari")
        ...
        if remaining == 0:
            fail(
                "Manuscript dropzone disappeared after a rejected upload on iOS Safari — "
                "rejection UX regression."
            )
        ok("Manuscript dropzone remains visible after rejection on iOS Safari")

        await page.screenshot(path=str(SCREENSHOTS / "docx_ios_rejection_success.png"))
        await browser.close()
        print("\niOS Safari unsupported-file rejection regression: all checks passed.")
