...
export function VerifyPdfUploadButton() {
  ...
  async function runVerify() {
    ...
    try {
      ...
      update("render", {
        state: "ok",
        detail: `${doc.numPages} page(s), ${Math.round(viewport.width)}×${Math.round(viewport.height)}`,
      });
            ...
            {steps.map((s) => (
              <li key={s.id} className="flex items-start gap-2 rounded-md border border-ink/10 bg-white px-2 py-1.5">
                <div className="mt-0.5">
                  ...
                  {s.state === "running" && <Loader2 className="h-4 w-4 animate-spin text-mute" />}
                  {s.state === "pending" && <div className="h-4 w-4 rounded-full border border-ink/20" />}
                </div>
                <div className="min-w-0 flex-1">
                  <div className="text-xs font-medium text-navy">{s.label}</div>
                  {s.detail && (
