...
import { finalizeCreatorStudioVideo, reserveCreatorStudioVideo } from "@/lib/creator-studio-entitlements.server";
...
export type CreatorStudioRenderJob = {
  id: string; owner_user_id: string; project_id: string; idempotency_key: string; provider: "SHOTSTACK";
  provider_environment: "stage" | "v1"; provider_render_id: string | null;
  status: "DRAFT" | "QUEUED" | "FETCHING" | "PREPROCESSING" | "RENDERING" | "SAVING" | "COMPLETED" | "FAILED" | "CANCELLED";
  output_url: string | null; failure_code: string | null; estimated_cost_cents: number; actual_cost_cents: number | null;
...
function mapStatus(status: ShotstackStatus): CreatorStudioRenderJob["status"] {
  return ({ queued: "QUEUED", fetching: "FETCHING", preprocessing: "PREPROCESSING", rendering: "RENDERING", saving: "SAVING", done: "COMPLETED", failed: "FAILED" } as const)[status];
}
function safeFailureCode(error: unknown) {
  const message = error instanceof Error ? error.message : "UNKNOWN";
  if (message === "CREATOR_STUDIO_QUOTA_EXCEEDED") return message;
...
export async function submitCreatorStudioRenderJob(params: { ownerUserId: string; projectId: string; idempotencyKey: string; callbackUrl?: string }) {
  ...
  const { data: existing } = await (supabaseAdmin.from("creator_studio_render_jobs" as never) as any).select(JOB_COLS).eq("owner_user_id", ownerUserId).eq("idempotency_key", idempotencyKey).maybeSingle();
  ...
  const { data: project, error: projectError } = await (supabaseAdmin.from("creator_studio_projects" as never) as any)
    .select("id,owner_user_id,goal,style_key,duration_seconds,product_title,call_to_action").eq("id", projectId).eq("owner_user_id", ownerUserId).maybeSingle();
  if (projectError || !project) throw new Error("CREATOR_STUDIO_PROJECT_NOT_FOUND");

  const plan = planCreatorStudioScenes({ goal: project.goal as CreatorStudioGoal, style: project.style_key as CreatorStudioStyle, durationSeconds: project.duration_seconds as 15 | 30 | 45, productTitle: project.product_title, callToAction: project.call_to_action });
  ...
  const { data: created, error: insertError } = await (supabaseAdmin.from("creator_studio_render_jobs" as never) as any)
    .insert({ owner_user_id: ownerUserId, project_id: projectId, idempotency_key: idempotencyKey, provider: "SHOTSTACK", provider_environment: environment, status: "DRAFT", render_plan: plan, estimated_cost_cents: estimateCostCents(plan.output.durationSeconds) })
    .select(JOB_COLS).single();
  if (insertError || !created) {
    const { data: raced } = await (supabaseAdmin.from("creator_studio_render_jobs" as never) as any).select(JOB_COLS).eq("owner_user_id", ownerUserId).eq("idempotency_key", idempotencyKey).maybeSingle();
  ...
  let reserved = false;
  try {
    await reserveCreatorStudioVideo(ownerUserId, idempotencyKey);
    reserved = true;
    const assets = await getSignedCreatorStudioAssets(projectId, ownerUserId);
    const provider = await submitShotstackRender(buildShotstackEdit(plan, assets, callbackUrl));
    const now = new Date().toISOString();
    const { data: queued, error: updateError } = await (supabaseAdmin.from("creator_studio_render_jobs" as never) as any)
      .update({ provider_render_id: provider.id, status: "QUEUED", submitted_at: now, updated_at: now, failure_code: null }).eq("id", created.id).eq("owner_user_id", ownerUserId).select(JOB_COLS).single();
    if (updateError || !queued) throw new Error("CREATOR_STUDIO_RENDER_JOB_UPDATE_FAILED");
    return queued as CreatorStudioRenderJob;
  } catch (error) {
    const now = new Date().toISOString();
    await (supabaseAdmin.from("creator_studio_render_jobs" as never) as any).update({ status: "FAILED", failure_code: safeFailureCode(error), completed_at: now, updated_at: now }).eq("id", created.id).eq("owner_user_id", ownerUserId);
    if (reserved) await finalizeCreatorStudioVideo(ownerUserId, idempotencyKey, false, created.id).catch(() => undefined);
    if (error instanceof Error && error.message === "CREATOR_STUDIO_QUOTA_EXCEEDED") throw error;
...
export async function refreshCreatorStudioRenderJob(params: { ownerUserId: string; jobId: string }) {
  const { data: job, error } = await (supabaseAdmin.from("creator_studio_render_jobs" as never) as any).select(`${JOB_COLS},provider_render_id`).eq("id", params.jobId).eq("owner_user_id", params.ownerUserId).maybeSingle();
  ...
  try {
    ...
    const { data: updated, error: updateError } = await (supabaseAdmin.from("creator_studio_render_jobs" as never) as any).update(patch).eq("id", job.id).eq("owner_user_id", params.ownerUserId).select(JOB_COLS).single();
    if (updateError || !updated) throw new Error("CREATOR_STUDIO_RENDER_JOB_UPDATE_FAILED");
    if (status === "COMPLETED") await finalizeCreatorStudioVideo(params.ownerUserId, job.idempotency_key, true, job.id);
    if (status === "FAILED") await finalizeCreatorStudioVideo(params.ownerUserId, job.idempotency_key, false, job.id);
    return updated as CreatorStudioRenderJob;
  } catch (error) {
    if (error instanceof Error && ["CREATOR_STUDIO_RENDER_JOB_UPDATE_FAILED", "CREATOR_STUDIO_USAGE_FINALIZE_FAILED"].includes(error.message)) throw error;
...
export async function refreshCreatorStudioRenderByProviderId(providerRenderId: string) {
  const { data: job, error } = await (supabaseAdmin.from("creator_studio_render_jobs" as never) as any).select("id,owner_user_id,provider_render_id").eq("provider", "SHOTSTACK").eq("provider_environment", getShotstackEnvironment()).eq("provider_render_id", providerRenderId).maybeSingle();
