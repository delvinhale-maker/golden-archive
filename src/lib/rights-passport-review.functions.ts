...
export const syncReviewFlags = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<ReviewFlagRow[]> => {
    ...
    const passport = passportRow as unknown as PassportRow;
    const passportKey = passport.passport_key;
    ...
    if (toInsert.length) {
      const { error } = await (supabase.from("rights_review_flags" as never) as any).insert(
        toInsert,
      );
