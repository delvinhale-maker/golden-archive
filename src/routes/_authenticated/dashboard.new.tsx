...
// category (see LEGACY_ALIAS in @/lib/categories) and offering both the
// old and new value side by side let sellers/admins accidentally file a
// product under the old bucket, where it would never surface on the
// newer category's department page. Existing products already on a
// legacy value keep working (see getQueryableSlugsFor) — this list only
// controls what a NEW or edited product can be set to going forward.
    ...
    <PublisherShell accent={accent}>
      ...
      <p className="text-mute mt-1">
        {isEditing
          ...
          : "A KDP-style flow. AurumVault keeps 15%; you keep 85%."}
      </p>
...
function buildChromeIntent(url: string): string | null {
  try {
    const u = new URL(url);
    const path = `${u.host}${u.pathname}${u.search}${u.hash}`;
    const fallback = encodeURIComponent(url);
    return `intent://${path}#Intent;scheme=${u.protocol.replace(":", "")};package=com.android.chrome;S.browser_fallback_url=${fallback};end`;
  } catch {
    return null;
  }
}
...
function FileInput({
  ...
  const openInChrome = () => {
    const intent = buildChromeIntent(window.location.href);
    if (intent) {
