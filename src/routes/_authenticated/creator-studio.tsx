...
/**
 * Creator Studio™ home. Customer-facing only — keep wording plain and
 * free of any backend/processing terminology (see the CS1 spec's UX rules).
 * The actual enabled/disabled behavior is enforced server-side by every
 * creator-studio.functions.ts entry point (requireCreatorStudioEnabled);
 * this page renders unconditionally and just tries the server functions,
 * which will fail closed with a plain message if the flag is off.
 */
    ...
    <PublisherShell accent={ACCENTS.creatorStudio}>
      <div className="mx-auto max-w-2xl text-center">
        <span className="inline-flex items-center gap-1.5 rounded-full border border-[#7A2E52]/30 bg-[#7A2E52]/5 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.14em] text-[#7A2E52]">
          <Clapperboard size={13} aria-hidden="true" /> Creator Studio
        </span>
        <h1 className="mt-4 font-display text-3xl leading-tight text-navy md:text-4xl">
          Turn your product into a polished promo video.
        </h1>
        <p className="mt-3 text-base text-mute md:text-lg">
          Choose what you want to promote. AurumVault handles the edit.
        </p>
        ...
        <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
          ...
          >
            <Film size={18} aria-hidden="true" /> My Videos
          </Link>
        </div>
      </div>
      ...
      <div className="mx-auto mt-14 grid max-w-3xl grid-cols-1 gap-4 sm:grid-cols-3">
        {[
          { title: "Pick your product", body: "Use an existing AurumVault product or upload one." },
          { title: "Choose a style", body: "Cinematic, Luxury, Bold Social, or Clean Minimal." },
          {
            title: "Review & save",
            body: "Your project is saved as a draft you can finish anytime.",
          },
        ].map((step) => (
